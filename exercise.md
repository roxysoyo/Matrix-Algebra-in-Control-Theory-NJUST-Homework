...
2.$ \quad \mathbf{U}_i \subset \mathbf{S},i \in \{1, 2\} \text{ 是 } \mathbf{S} \text{ 的真子空间，}
...
& \text{一种几何解释：S为平面，}U_1\text{和}U_2\text{是横轴和纵轴。}
\end{aligned}
$$
...
证明下述结论：
1.$ \textbf{ R}( A+ B) \subset \mathbf{R} ( A) + \mathbf{R} ( B) .$
2.$ \textbf{ N}( A) \cap \mathbf{N} ( B) = \mathbf{N} ( A+ B) \cap \mathbf{N} ( A- B) .$
3.$rank(A+B)\leq rank(A)+rank(B)$，并举例说明该结论的等号与不等号均可能实现.

以上1-3，设$A$与$B$是同行列数的.

4.$A,B\in\mathbf{C}^{n\times n},AB=0$，则 $rank(A)+rank(B)\leq n$.

5.$A\in\mathbf{C}^{n\times n}$，则 $rank(A)=1$当且仅当有$a,b\in\mathbf{C}^n$均非零，使$A=ab^T$.
6.$A, B\in \mathbf{C} ^{n\times n}$，则 $|rank(A)-rank(B)|\leq rank(A+B)$.
...
I_m & 0 \\
0 & 0
\end{pmatrix}, \quad B \in \mathbf{C}^{n \times n}. \\
&O = PAB, \\
&\quad = 
\begin{pmatrix}
I_{r_A} & 0 \\
...
&\text{令 } V = \mathbf{R}(B), \text{ 根据定理 1.8.2 得证.}\,\text{见书上 P}_{26}.
\end{aligned}
$$
...
$$
\begin{aligned}
&\sigma, \mathbf{S}, \mathbf{T}, \mathbf{Q}, \mathbf{U} \quad \text{如定义 1.8.2, 则：} \\
